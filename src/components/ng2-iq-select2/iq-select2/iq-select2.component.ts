/* tslint:disable */
import {
    AfterViewInit, ChangeDetectorRef, Component, ElementRef, EventEmitter, forwardRef, Input, Output, TemplateRef,
    ViewChild
} from '@angular/core';
...
export class IqSelect2Component implements AfterViewInit, ControlValueAccessor {
    ...
    @Input() dataSourceProvider: (term: string) => Observable<any[]>;
    ...
    @Input() noSearchBox: boolean = false;
    ...
    private subscribeToResults(observable: Observable<string>): void {
        setTimeout(() => {
            observable
                .do(() => { this.resultsVisible = false; this.searching = true; })
                ...
                .do(() => { this.resultsVisible = this.searchFocused; this.searching = false; })
                .subscribe((items) => {
                    this.listData = items;
    ...
    private containsText(item: any, term: string) {
